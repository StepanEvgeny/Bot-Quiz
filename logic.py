...
def start_quiz(user_id):
    """Инициализация теста для пользователя."""
    user_progress[user_id] = 0
    return ask_question(user_id)

def ask_question(user_id):
    """Отправка вопроса пользователю."""
    question_num = user_progress.get(user_id, 0)
    if question_num >= len(questions):
        return None, None, FSInputFile(MEME_PATH), "На что ты потратил эти часы? 😂"
    
    question, options, _ = questions[question_num]
    return question, get_keyboard(options), None, None
...
def check_answer(user_id, answer):
    ...
    if answer == correct_answer:
        ...
        user_progress[user_id] = 0
        return start_quiz(user_id)  # Начинаем заново
